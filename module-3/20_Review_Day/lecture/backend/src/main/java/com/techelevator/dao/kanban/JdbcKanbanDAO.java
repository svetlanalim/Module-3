...
public class JdbcKanbanDAO implements KanbanDAO {
    ...
    private Card mapRowToCard(SqlRowSet rowSet) {
        ...
        Date creationDate = rowSet.getDate("creation_date");
        if(creationDate != null) result.setDate(creationDate.toLocalDate());
    ...
    public Card createCard(long boardId, Card card) {
        String sql = "INSERT INTO cards (board_id, title, description, avatar, status, tag) " +
                     "VALUES (?, ?, ?, ?, ?, ?) RETURNING id;";
        Long newId = jdbcTemplate.queryForObject(sql, Long.class, boardId, card.getTitle(), card.getDescription(),
                card.getAvatar(), card.getStatus(), card.getTag());
        card.setId(newId);
